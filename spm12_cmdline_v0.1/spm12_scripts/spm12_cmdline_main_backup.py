...
m_script_archive = "/data/eck/CustomScriptsArchive/spm12_scripts"
...
parser_run.add_argument("-s", "--scan-types", default = ['t1ce', 't2', 'seg'], 
                        help="A list of available image scan types.(e.g. -s t1ce t2 seg) The last element of this should be 'seg'. Otherwise it will malfunction.",
                        type=str, nargs='+')
parser_run.add_argument("-l", "--log-file", default = "spm12_cmdline.log", 
                        help="log file.",
                        type=str, nargs='?')
    ...
    def set_input_dict(self, root_folder, img_types, pid):
        ...
        for i in input_dict:
            ...
            for j in input_dict[i]:
                ...
                for t in img_types:
                    if t != 'seg':
                        tl = if_found_return_groups("^((?!roi|seg).)*$",
                                                        if_found_return_groups(f"{root_folder}/{i}/{j}/{t}.*/.*{self.fep}", gr1, 0, re.I),
                                                        0,
                                                        re.I)
                        if len(tl) == 1:
                            td[t] = tl[0]
                        else:
                            raise ValueError(f"More than one path was found. {tl}")

                    else:
                        tl = if_found_return_groups(f"{root_folder}/{i}/{j}/.*/.*(?:roi|seg).*{self.fep}", gr1, 0, re.I)
                        if len(tl) == 1:
                            td[t] = tl[0]
                        else:
                            raise ValueError(f"More than one path was found. {tl}")
...
def convert_mscript(process, inputs):
    ...
    with open(f"{m_script_archive}/{process}.m", 'r') as f:
            contents = f.read()
...
def _run_m_script(m_script):
    cmd1 = f'''run_spm12.sh /data/eck/software/MCR/v97 script {m_script}'''
...
if args.command == 'convert':
    ...
    if (len(args.patient_id) > 1) or (args.patient_id == ['all']):
        ...
        for pid in pids:
            print(f'''python /data/eck/CustomScriptsArchive/spm12_scripts/spm12_cmdline_main.py run {args.root_folder} {pid} -s {' '.join(args.scan_types)} -l {args.root_folder}/{pid}/{pid}_spm12_run.log'''.split())
            subpcs.append(
                Popen(f'''python /data/eck/CustomScriptsArchive/spm12_scripts/spm12_cmdline_main.py run {args.root_folder} {pid} -s {' '.join(args.scan_types)} -l {args.root_folder}/{pid}/{pid}_spm12_run.log'''.split(), stdout = PIPE, stderr = PIPE, cwd = os.getcwd())
            )
